...
SOURCEFILE="https://github.com/NagiosEnterprises/nrpe/archive/nrpe-4.0.3.tar.gz"
PLUGINTARFILE="https://nagios-plugins.org/download/nagios-plugins-2.3.3.tar.gz"
DESTPATH="/root/gmetricsdata/"
#FILEUNTAR=`tar -tvf nrpe*tar.gz | head -n 1 | awk '{print $6}'`
#UNTARPATH=""$DESTPATH""$FILEUNTAR""
#PLUGINPATH=`tar -tvf nagios-plugins*tar.gz | head -n 1 | awk '{print $6}'`
#PLUGINTARPATH=""$DESTPATH""$PLUGINPATH""
...
package_install_centos8 () {
    ...
    yum install -y epel-release gcc glibc glibc-common make gettext automake autoconf wget openssl-devel net-snmp net-snmp-utils
...
gmetrics_customisation () {
    ...
    NRPEFILE=""$DESTPATH"nrpe-nrpe-4.0.3/src/nrpe.c"
    echo "#######################################################" | log
...
check_metrics_customization () {
    ...
    CHECK_NRPEFILE=""$DESTPATH"nrpe-nrpe-4.0.3/src/check_nrpe.c"
    echo "#######################################################" | log
...
agent_service_customisation_centos () {
    ...
    echo "#######################################################" | log
    echo "Rename gmetrics-agent service file" | log
    cp  /usr/lib/systemd/system/nrpe.service /usr/lib/systemd/system/gmetrics-agent.service | log
    echo "
[Unit]
Description=Groots Agent Remote Plugin Executor
Documentation=https://www.groots.in/
...
start_gmetrics_agent () {
    ...
    echo "#######################################################" | log
    echo "Verify gmetrics-agent config. Checking agent version.."
    /groots/metrics/libexec/check_metrics -H 127.0.0.1 | log
}
...
download_plugin_source () {
	...
	PLUGINPATH=`tar -tvf "$DESTPATH"nagios-plugins*tar.gz | head -n 1 | awk '{print $6}' | sed -e 's|/||g'`
	PLUGINTARPATH=""$DESTPATH""$PLUGINPATH""
    tar -xvf "$DESTPATH"nagios-plugins*.tar.gz -C $DESTPATH | log

    if [ ! -d $PLUGINTARPATH ]; then
            echo "#######################################################" | log
